...
1. **Single emitter.**  Both ``GET /api/tasks`` and the periodic WebSocket
   broadcast in :class:`citrasense.web.app.CitraSenseWebApp` need the same
...
from citrasense.web.helpers import _task_to_dict

_logger = logging.getLogger("citrasense.SkyEnrichment")
